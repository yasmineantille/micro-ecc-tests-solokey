...
/*void uEccPoint_mult(uint8_t* private_key, uint8_t* x, uint8_t* y, uint8_t* result_x, uint8_t* result_y, uECC_Curve curve)
{
    // Define the private key
    uint8_t private_key[uECC_BYTES];
    // Fill the private key with random bytes or derive it from a seed
    // ...

    // Generate the public key by point multiplication
    uint8_t public_key[uECC_BYTES * 2];
    EccPoint public_point;
    EccPoint_mult(&curve, &public_point, curve.G, private_key, NULL);
    uECC_compress(&curve, public_point, public_key);

    // Copy the resulting x and y coordinates to the output buffers
    memcpy(result_x, &public_key[1], uECC_BYTES);
    memcpy(result_y, &public_key[1+uECC_BYTES], uECC_BYTES);
}*/


// Secure Auth helper functions
                               ...
                               uECC_Curve curve)
{
    ...
    // Convert point and scalar to proper format
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
    printf("Reached uECC_VLI_NATIVE_LITTLE_ENDIAN for point conversion\n");
    ...
    uECC_vli_bytesToNative(_point + curve->num_words, point + num_bytes, num_bytes);
#endif

    // make sure point is valid first
    if (!uECC_valid_point(_point, curve)) {
        return 2;
    }

    // Make sure scalar is in the range [1, n-1]
    if (uECC_vli_isZero(_scalar, curve->num_words)) {
        return 5;
    }

    // Perform scalar multiplication
    uECC_word_t initial_Z = 0;
    EccPoint_mult(_result, _point, _scalar, &initial_Z, num_n_bits, curve);

    // Make sure resulting point is on curve
    // Currently here error is thrown
    ...
    // TODO: THIS IS THE LINE TO FIX
    uECC_vli_nativeToBytes(result + num_bytes, num_bytes, _result + curve->num_words);
#endif
                  ...
                  uECC_Curve curve)
{
    ...
    uECC_vli_bytesToNative(X1, x, num_bytes);
    uECC_vli_bytesToNative(Y1, x + num_bytes, num_bytes);
    uECC_vli_bytesToNative(X2, y, num_bytes);
    uECC_vli_bytesToNative(Y2, y + num_bytes, num_bytes);
